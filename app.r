...
top finishes in playoffs")
    ...
    tags$p("Regrettably, the numbers clearly show that the #1 GM over the course of his tenure is Nik. With a 2nd, 5th and two championships to his name, this makes sense.
           We should note that his latest victory is mired in controversy with allegations of collusion between disgraced former GM Clem.")
    )
)
...
server <- function(input, output) {
  ...
  results <- results %>% group_by(YEAR) %>% mutate(rank_pct2 = (n()/RANK)^(input$num))
  
  
  results_team <- results %>% group_by(TEAM) %>% summarize(seasons = n(),
                                                           max_rank = min(RANK),
                                                           min_rank = max(RANK),
                                                           pts=sum(PTS),
                                                           avg_playoff_score = sum(rank_pct2)/n(),
                                                           max_pts = sum(MAX_PTS), 
                                                           min_max_pts_percentage = min(MAX_PTS_PERCENTAGE), 
                                                           avg_max_pts_percentage = sum(PTS)/sum(MAX_PTS),
                                                           max_max_pts_percentage = max(MAX_PTS_PERCENTAGE)) %>% 
